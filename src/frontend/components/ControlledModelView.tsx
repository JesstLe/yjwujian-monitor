...
export interface ExportParams {
  frameUrl: string;
  containerRect: { w: number; h: number };
  effectiveScale: number;
  effectivePan: { x: number; y: number };
  frameAspectRatio: number;
}

export interface ControlledModelViewRef {
...
export default forwardRef<ControlledModelViewRef, ControlledModelViewProps>(
  function ControlledModelView({
    captureUrls,
    fallbackUrl,
    name,
    serialNum,
    ...
    useImperativeHandle(ref, () => ({
      getVisibleContainer: () => visibleContainerRef.current,
      getExportParams: (): ExportParams | null => {
        const container = visibleContainerRef.current;
        if (!container) return null;

        // 当前帧 URL
        const currentFrameIndex = hasCapture ? angleToFrameIndex(angle) : 0;
        const frameUrl = hasCapture
          ? proxiedCaptureUrls[currentFrameIndex]
          : proxiedFallbackUrl;
        ...
        return {
          ...
          effectiveScale: scale * localScale,
          effectivePan: {
            x: globalPan.x + localPan.x,
            y: globalPan.y + localPan.y,
          },
    ...
    }), [angle, scale, localScale, globalPan, localPan, frameAspectRatio, hasCapture, proxiedCaptureUrls, proxiedFallbackUrl]);
