    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-2">
          {isListing || onClick ? (
            <h3 className="font-semibold text-gray-900 line-clamp-1 flex-1 group-hover:text-blue-600 transition-colors" title={item.name}>
              {item.name}
            </h3>
          ) : (
          ...
          <span className={`flex-shrink-0 px-2.5 py-1 text-xs font-semibold rounded-lg border ${rarity.badgeClass}`}>
            {rarity.label}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          {isListing ? (
            <span className="flex items-center gap-1 text-xs text-gray-400" title="收藏人数">
              {Icons.heart}
              {item.collectCount}
            </span>
          ) : (
